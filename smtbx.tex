...
\subsection{ADP U volume similarity}
...
\begin{equation}
U = Q\begin{pmatrix}\lambda_{1}&0&0\\0&\lambda_{2}&0\\0&0&\lambda_{3}\end{pmatrix} Q^{-1}.
\end{equation}
Giving us the following equation for determining the eigenvalues:
\begin{equation}
\begin{pmatrix}\lambda_{1}&0&0\\0&\lambda_{2}&0\\0&0&\lambda_{3}\end{pmatrix} = Q^{-1}UQ.
\end{equation}
and for each $\lambda$ the expression is:
\begin{equation}
\lambda_{i} = \sum_{j}^3(x_{j}*Q_{ji}) \text{ with } x_{j} = \sum_{k=1}^3(Q^{-1}_{jk}U_{kj})
\end{equation}
And derivatives of the residual with respect to elements of U are:
\begin{equation}
\partialder{L_{A}}{U_{ij}} = w \partialder{V_{A}}{U_{ij}} = \frac{4w\pi}{6\sqrt[2]{\lambda_{1}\lambda_{2}\lambda_{3}}} \sum_{i=1}^3(\prod_{j=1, j \neq i}^3\lambda_{j}\partialder{\lambda_{i}}{U_{ij}})
\end{equation}
which then expands to:
\begin{equation}
\partialder{L_{A}}{U_{ij}} = \frac{4w\pi}{6\sqrt[2]{\lambda_{1}\lambda_{2}\lambda_{3}}} \sum_{i=1}^3(\prod_{k=1, k \neq i}^3\lambda_{k}Q_{ki}Q_{kj})
\end{equation}
