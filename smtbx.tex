...
Olex2.refine uses crystallographic information file (CIF, \cite{Hall:1991aa}) as its main output. The CIF contains information regarding the space group, the data indicators such as merging indices, the refinement indicators as the R factors, goodness of fit, residual electron density, refinement convergence indicators and tabulated structure information including tables of atomic parameters, bonds and angles. In the case of restrained refinement Olex2.refinement produces a tables describing the restraints using the CIF restraints dictionary. Olex2.refine CIF always contains a verbal description of the refinement model - hydrogen atom treatment, constraints, restraints and their targets. Optionally, the refinement model file (Shelx RES file) and the reflections can also be include into the final CIF for the deposition. As a part of this work a CIF handling toolbox \cite{Gildea:2011aa} was added to cctbx.
...
Olex2.refine can refine extinction parameter defined identically to the one used in Shelxl:
\begin{align}
F_c^{'} = F_c(1+0.001\frac{F_c^{2}\lambda^3}{sin(2\theta)})^{-\frac{1}{4}} 
\end{align}
It is mentioned in the Shelxl manual that this parameter is not expressed exactly as in literature and is  empirical.
For theory behind extinction see for example \cite{Becker:1974aa}.
